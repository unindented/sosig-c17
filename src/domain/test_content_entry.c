...
// always have entries. Without this test, `LATEST_DATE_FALLBACK` could change without a failure. An
// invalid `<updated>` value would then appear in every feed for an empty site. Both expected values
// came from a run.

...
// Slugifies a whole terminated string, discarding the reported length, so the cases below read
// without a length at every call. `test_slugify_reports_length` covers the length itself.

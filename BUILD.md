...
| `tests/CMakeLists.txt` | Add the golden test suite |
...
Each source directory adds its local unit tests. The top-level `tests/` directory adds only the golden test suite. Each CTest case has a `sosig` label. It also has a `unit` or `golden` label.

...
// Parses `x = <value>` into `result_out`, which the caller owns and must `toml_free`, and returns
// the datum bound to `x` that borrows from it.
...
static void test_rejects_non_datetime_values(void) {
  ...
  TEST_CHECK(toml_datum_to_epoch(time_value, &time_epoch) != 0);

...
// Creates a temporary directory root for filesystem tests. Returns `NULL` as a failure sentinel
// that every caller branches on with `if (tmp == NULL) return;`. That guard is not a silent skip,
// because the `TEST_CHECK` here has already failed the test and the guard only suppresses cascading
// noise. The returned pointer aliases the caller's own `dir` array rather than being owned, so a
// caller must not free it and must keep `dir` alive for the whole test.
static const char* init_fs_fixture(char root_dir[static 1]) {
  char* tmp = mkdtemp(root_dir);
  TEST_CHECK(tmp != NULL);
  return tmp;
}

// Builds the reason a `fs` action reports for `error_number`: the system message, relayed exactly
// as the system spells it. Derived from the running libc rather than hardcoded, so the assertion
// states the whole contract and still holds where the wording differs.
static const char* expected_errno_reason(char buf[static FS_REASON_SIZE], int error_number) {
  (void)snprintf(buf, FS_REASON_SIZE, "%s", strerror(error_number));
  return buf;
}
...
static void test_list_files_matches_suffix(void) {
  char dir[] = "/tmp/sosig-fs-suffix-match.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* nested_md = path_join(tmp, "nested/b.md", &arena);
  char* root_md = path_join(tmp, "a.md", &arena);
  char* ignored = path_join(tmp, "nested/c.txt", &arena);
  ...
  TEST_CHECK(fs_list_files_with_suffix(&paths, tmp, ".md", reason, sizeof(reason)) == 0);
  ...
  (void)unlink(root_md);
  (void)rmdir(path_join(tmp, "nested", &arena));
  (void)rmdir(tmp);
  arena_free(&arena);
}
...
static void test_list_files_empty_suffix_matches_all(void) {
  char dir[] = "/tmp/sosig-fs-suffix.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* md_path = path_join(tmp, "a.md", &arena);
  char* txt_path = path_join(tmp, "b.txt", &arena);
  TEST_CHECK(fs_write_file(md_path, "a", 1, NULL, 0) == 0);
  TEST_CHECK(fs_write_file(txt_path, "b", 1, NULL, 0) == 0);

  struct PathList paths;
  path_list_init(&paths);
  TEST_CHECK(fs_list_files_with_suffix(&paths, tmp, "", NULL, 0) == 0);
  TEST_CHECK(paths.count == 2);
  path_list_free(&paths);
...
static void test_list_files_accepts_empty_dir(void) {
  char dir[] = "/tmp/sosig-fs-list-empty.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct PathList paths;
  path_list_init(&paths);
  TEST_CHECK(fs_list_files_with_suffix(&paths, tmp, ".md", NULL, 0) == 0);
...
static void test_list_files_skips_symlink_cycle(void) {
  char dir[] = "/tmp/sosig-fs-list.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* page = path_join(tmp, "page.md", &arena);
  char* loop = path_join(tmp, "loop", &arena);
...
static void test_list_files_skips_symlink_cycle_to_ancestor(void) {
  char dir[] = "/tmp/sosig-fs-list-ancestor.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* page = path_join(tmp, "sub/page.md", &arena);
  char* up = path_join(tmp, "sub/up", &arena);
  TEST_CHECK(fs_write_file(page, "x", 1, NULL, 0) == 0);
  ...
  (void)unlink(up);
  (void)unlink(page);
  (void)rmdir(path_join(tmp, "sub", &arena));
  arena_free(&arena);
  (void)rmdir(tmp);
}
...
static void test_list_files_rejects_missing_dir(void) {
  char dir[] = "/tmp/sosig-fs-list-missing.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* missing = path_join(tmp, "does-not-exist", &arena);
  ...
  (void)snprintf(expected, sizeof(expected), "cannot inspect directory: %s ('%s')",
...
static void test_list_files_rejects_file_root(void) {
  char dir[] = "/tmp/sosig-fs-list-file-root.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* plain = path_join(tmp, "plain.txt", &arena);
  ...
  char expected[FS_REASON_SIZE];
  char message[FS_REASON_SIZE];
  (void)snprintf(expected, sizeof(expected), "cannot open directory: %s ('%s')",
...
static void test_list_files_rejects_unstatable_entry(void) {
  ...
  char dir[] = "/tmp/sosig-fs-list-unstatable.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* sealed_dir = path_join(tmp, "sealed", &arena);
  char* sealed_md = path_join(tmp, "sealed/a.md", &arena);
  ...
  path_list_init(&paths);
  char reason[FS_REASON_SIZE] = "";
  const int rc = fs_list_files_with_suffix(&paths, tmp, ".md", reason, sizeof(reason));
  ...
  char message[FS_REASON_SIZE];
  (void)snprintf(expected_entry, sizeof(expected_entry), "cannot inspect entry: %s ('%s')",
...
static void test_read_file_accepts_empty(void) {
  char dir[] = "/tmp/sosig-fs-empty.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* empty_path = path_join(tmp, "empty.md", &arena);
...
static void test_read_file_rejects_missing_and_non_regular(void) {
  char dir[] = "/tmp/sosig-fs-read.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* missing = path_join(tmp, "missing.md", &arena);
...
static void test_read_file_rejects_embedded_nul(void) {
  char dir[] = "/tmp/sosig-fs-nul.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* nul_path = path_join(tmp, "nul.md", &arena);
...
static void test_write_then_read_round_trips(void) {
  char dir[] = "/tmp/sosig-fs-roundtrip.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
...
static void test_write_file_rejects_file_parent_and_dir_target(void) {
  char dir[] = "/tmp/sosig-fs-write-fail.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  char* blocking_file = path_join(tmp, "plain.txt", &arena);
  char* through_file = path_join(tmp, "plain.txt/child.html", &arena);
  TEST_CHECK(fs_write_file(blocking_file, "x", 1, NULL, 0) == 0);
  char reason[FS_REASON_SIZE] = "";
  TEST_CHECK(fs_write_file(through_file, "x", 1, reason, sizeof(reason)) == -1);
  char expected[FS_REASON_SIZE];
  (void)snprintf(expected, sizeof(expected), "exists and is not a directory ('%s')", blocking_file);
  TEST_CHECK(strcmp(reason, expected) == 0);
  ...
  char* dir_target = path_join(tmp, "sub", &arena);
...
static void test_mkdir_p_creates_nested_and_is_idempotent(void) {
  char dir[] = "/tmp/sosig-fs-mkdir.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  char* nested = path_join(tmp, "x/y/z", &arena);
  ...
  (void)rmdir(path_join(tmp, "x/y/z", &arena));
  (void)rmdir(path_join(tmp, "x/y", &arena));
  (void)rmdir(path_join(tmp, "x", &arena));
  (void)rmdir(tmp);
  arena_free(&arena);
}
...
static void test_mkdir_p_rejects_file_component(void) {
  char dir[] = "/tmp/sosig-fs-mkdir-fail.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  char* existing_file = path_join(tmp, "file", &arena);
  TEST_CHECK(fs_write_file(existing_file, "x", 1, NULL, 0) == 0);
  char reason[FS_REASON_SIZE] = "";
  TEST_CHECK(fs_mkdir_p(existing_file, reason, sizeof(reason)) == -1);
  char expected[FS_REASON_SIZE];
  (void)snprintf(expected, sizeof(expected), "exists and is not a directory ('%s')", existing_file);
  TEST_CHECK(strcmp(reason, expected) == 0);
  ...
  char* path_through_file = path_join(tmp, "file/child", &arena);
  reason[0] = '\0';
...
static void test_mkdir_p_rejects_unstatable_component(void) {
  char dir[] = "/tmp/sosig-fs-mkdir-unstatable.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  char* dangling = path_join(tmp, "dangling", &arena);
...
static void test_mkdir_p_rejects_uncreatable_component(void) {
  ...
  char dir[] = "/tmp/sosig-fs-mkdir-refused.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  arena_init(&arena);
  char* sealed_dir = path_join(tmp, "sealed", &arena);
  char* child_dir = path_join(tmp, "sealed/child", &arena);
  ...
  char expected[FS_REASON_SIZE];
  char message[FS_REASON_SIZE];
  (void)snprintf(expected, sizeof(expected), "cannot create directory: %s ('%s')",
...
static void test_identify_distinguishes_files_and_rejects_missing(void) {
  char dir[] = "/tmp/sosig-fs-identify.XXXXXX";
  const char* tmp = init_fs_fixture(dir);
  if (tmp == NULL) {
    return;
  }
  ...
  struct Arena arena;
  arena_init(&arena);
  char* file_path = path_join(tmp, "a.md", &arena);
  char* dotted_path = path_join(tmp, "./a.md", &arena);
  char* other_path = path_join(tmp, "b.md", &arena);
  ...
  TEST_CHECK(fs_identify(path_join(tmp, "missing.md", &arena), &unwritten) == -1);
